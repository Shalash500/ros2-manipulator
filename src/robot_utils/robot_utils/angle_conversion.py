...
from custom_msgs.srv import QuaternionToEuler, EulerToQuaternion
...
class AngleConverter(Node):
    ...
    def eulerToQuaternionCallback(self, req, res):
        self.get_logger().info(f"Requested to convert euler angles roll:{req.roll}, pitch:{req.pitch}, yaw:{req.yaw} in to quaternion angles")
        (res.x, res.y, res.z, res.w) = quaternion_from_euler(req.roll, req.pitch, req.yaw)
        self.get_logger().info(f"Corresponding quaternion angle x:{res.x}, y:{res.y}, z:{res.z}, w:{res.w}")
        return res

    def quaternionToEulerCallback(self, req, res):
        self.get_logger().info(f"Requested to convert quaternion angles x:{req.x}, y:{req.y}, z:{req.z}, w:{req.w} in to quaternion angles")
